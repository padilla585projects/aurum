...
import { fromBase64Url, hashToken, randomId, randomToken, verifyPayload, timingSafeEqual } from '../../../_lib/crypto.ts';
...
/** Esquema propio de la APK. Debe coincidir con el intent-filter de Android. */
const NATIVE_SCHEME = 'aurum://auth';
const EXCHANGE_TTL_MS = 2 * 60 * 1000;

/**
 * Vuelve a la aplicación nativa. Nunca se manda el token de sesión por aquí:
 * en Android otra aplicación puede registrar el mismo esquema y recibir esta
 * URL. Lo que viaja es un código de un solo uso que la app canjea por la sesión
 * en POST /api/auth/exchange.
 */
function backToApp(params: Record<string, string>): Response {
  const url = new URL(NATIVE_SCHEME);
  ...
  return new Response(null, {
    ...
    headers: {
      ...
      'Set-Cookie': `${OAUTH_COOKIE}=; Path=/api/auth/google; HttpOnly; Secure; SameSite=Lax; Max-Age=0`,
    },
  });
}
...
export async function onRequestGet(context: PagesContext): Promise<Response> {
  ...
  // Desde aquí ya sabemos por dónde hay que devolver al usuario: a la web o a
  // la aplicación nativa que abrió el navegador del sistema.
  const finish = (params: Record<string, string>): Response =>
    state.native ? backToApp(params) : backTo(base, params);
  ...
  if (!tokenRes.ok || !tokens?.id_token) {
    ...
    return finish({ auth: 'error', reason: 'exchange_failed' });
  }
  ...
  if (!sub || !email) {
    return finish({ auth: 'error', reason: 'no_claims' });
  }
  ...
  if (link) {
    ...
    if (!row) return finish({ auth: 'error', reason: 'unknown_user' });
    if (row.status !== 'active') return finish({ auth: 'error', reason: 'suspended' });
  ...
  if (!user) {
    ...
    if (existing) {
      if (!emailVerified) {
        ...
        return finish({ auth: 'error', reason: 'email_unverified' });
      }
      if (existing.status !== 'active') return finish({ auth: 'error', reason: 'suspended' });
  ...
  if (!user) {
    ...
    if (!invite.ok) {
      ...
      return finish({ auth: 'error', reason: 'invite_invalid' });
    }
    ...
    if (!(await consumeInvite(env, invite.codeHash, created.id))) {
      ...
      return finish({ auth: 'error', reason: 'invite_used' });
    }
  ...
  // En la APK no se crea la sesión aquí: se emite un vale de un solo uso y la
  // propia aplicación lo canjea, para que el token no viaje en el deep link.
  if (state.native) {
    const code = randomToken(32);
    const now = Date.now();
    await env.DB.prepare(
      `INSERT INTO auth_exchange_codes (code_hash, user_id, created_at, expires_at)
       VALUES (?, ?, ?, ?)`,
    ).bind(await hashToken(code), user.id, now, now + EXCHANGE_TTL_MS).run();
    await audit(env, { userId: user.id, event: 'oauth_native_code_issued', route: '/api/auth/google/callback', status: 200, ip });
    return backToApp({ auth: 'ok', code });
  }
