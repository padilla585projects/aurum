...
const RESEARCH_STEPS = [
  { label:"Noticias y catalizadores",   q: a => `${a} noticias últimas semanas novedades importantes 2025` },
  { label:"Resultados y finanzas",      q: a => `${a} resultados financieros revenue beneficios earnings guidance` },
  { label:"Análisis de analistas",      q: a => `${a} precio objetivo analistas consenso recomendación buy sell` },
  { label:"Contexto macro y sector",    q: a => `sector ${a} tendencias perspectivas oportunidades 2025` },
  { label:"Riesgos y factores bajistas",q: a => `${a} riesgos amenazas problemas competencia bajistas bear case` },
];

/* ══════════════════════════════════════════════════════════════
   SYSTEM PROMPTS
══════════════════════════════════════════════════════════════ */
const makeAurumSys = (profile, portfolio) => {
...
/* ══════════════════════════════════════════════════════════════
   API UTILITY
══════════════════════════════════════════════════════════════ */
const callApi = async (messages, system, onSearch) => {
  let cur = [...messages];
  for (let i = 0; i < 8; i++) {
    ...
    if (data.stop_reason === "tool_use") {
      ...
      cur.push({ role: "user", content: data.content.filter(b => b.type === "tool_use").map(b => ({ type:"tool_result", tool_use_id:b.id, content:"Search executed" })) });
    } else {
      return data.content.filter(b => b.type === "text").map(b => b.text).join("\n").trim() || "Sin respuesta.";
    }
  }
...
/* ══════════════════════════════════════════════════════════════
   SHARED COMPONENTS
══════════════════════════════════════════════════════════════ */
const inputBase = { background:C.surf2, border:`1px solid ${C.border2}`, borderRadius:9, padding:"8px 11px", color:C.text, fontSize:".82em", fontFamily:"'Sora',sans-serif", outline:"none", width:"100%", transition:"border-color .2s" };
...
/* ══════════════════════════════════════════════════════════════
   CHAT TAB
══════════════════════════════════════════════════════════════ */
function ChatTab({ profile, portfolio }) {
  ...
  const [apiHists, setApiHists] = useState({ aurum:[], macro:[], riesgo:[], fiscal:[] });
  ...
  const handleFile = (e) => {
    ...
    reader.onload = () => setFile({ name:f.name, type:f.type, b64:reader.result.split(",")[1] });
  ...
  const send = async (override) => {
    ...
    // Build display message & api content
        ...
        {Object.entries(AGENTS).map(([key, ag]) => {
          ...
          return (
            <button key={key} className="agent-tab" onClick={() => setAgentKey(key)}
              ...
              <span>{ag.icon}</span>
              <span>{ag.name}</span>
              {active && <span style={{ fontSize:".75em", opacity:.65, display:"none" }}>{ag.desc}</span>}
            </button>
          );
        })}
        <div style={{ flex:1 }} />
...
function PortfolioTab({ portfolio, setPortfolio }) {
  ...
  const save = async (updated) => { setPortfolio(updated); await sSet("aurum-portfolio", updated); };
  ...
  const remove = (id) => save(portfolio.filter(p => p.id !== id));
  ...
  const fieldStyle = { ...inputBase, padding:"7px 10px" };
          ...
          {adding && (
            <div style={{ padding:"14px 16px", background:"#0a0a18", borderBottom:`1px solid ${C.border}` }}>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8, marginBottom:10 }}>
                {[["ticker","Ticker","AAPL"],["name","Nombre","Apple Inc."],["shares","Acciones","10"],["avgPrice","P.Compra €","150"],["currentPrice","P.Actual €","185"]].map(([k,l,ph]) => (
                  <div key={k}>
                ...
                {portfolio.map(p => {
                  ...
                  return (
                    <div key={p.id} className="pos-row" style={{ display:"grid", gridTemplateColumns:"80px 1fr 60px 75px 75px 90px 36px", padding:"11px 16px", borderBottom:`1px solid ${C.border}22`, fontSize:".82em", alignItems:"center", transition:"background .15s" }}>
                      ...
                      <button onClick={()=>remove(p.id)} style={{ background:"transparent", border:"none", color:"#252540", cursor:"pointer", fontSize:".9em", borderRadius:6, padding:4, transition:"color .15s" }} onMouseEnter={e=>e.target.style.color=C.red} onMouseLeave={e=>e.target.style.color="#252540"}>✕</button>
                    </div>
                  );
                })}
              </>
          ...
          <Card style={{ padding:"16px" }}>
            ...
            <ResponsiveContainer width="100%" height={170}>
              <PieChart>
                ...
                <Tooltip formatter={v=>[`${v.toLocaleString("es-ES")}€`]} contentStyle={{ background:C.surf3, border:`1px solid ${C.border}`, borderRadius:8, fontSize:".76em", color:C.text }} />
              </PieChart>
            </ResponsiveContainer>
...
function ResearchTab() {
  ...
  const run = async () => {
    ...
    try {
      ...
      setReport(synthesis);
    } catch(e) { setReport(`⚠️ Error en síntesis: ${e.message}`); }
...
function SimulatorTab() {
  ...
  const sliders = [
    { label:"Capital inicial",      val:initial, set:setInitial, min:0,  max:100000, step:500, fmt:v=>`${v.toLocaleString("es-ES")}€` },
    { label:"Aportación mensual",   val:monthly, set:setMonthly, min:0,  max:2000,   step:50,  fmt:v=>`${v}€/mes` },
    { label:"Rentabilidad esperada",val:rate,    set:setRate,    min:1,  max:20,     step:.5,  fmt:v=>`${v}% anual` },
    { label:"Horizonte temporal",   val:years,   set:setYears,   min:1,  max:40,     step:1,   fmt:v=>`${v} años` },
  ];
        ...
        <div style={{ flex:1, minHeight:0 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top:10, right:20, left:10, bottom:0 }}>
              ...
              <XAxis dataKey="año" stroke={C.muted} tick={{ fontSize:11, fill:C.muted }} tickFormatter={v=>`A${v}`} />
              <YAxis stroke={C.muted} tick={{ fontSize:11, fill:C.muted }} tickFormatter={v=>v>=1000?`${(v/1000).toFixed(0)}k€`:`${v}€`} />
              <Tooltip contentStyle={{ background:C.surf3, border:`1px solid ${C.border}`, borderRadius:9, fontSize:".78em" }} formatter={v=>[`${v.toLocaleString("es-ES")}€`]} labelFormatter={v=>`Año ${v}`} />
